...
<table id="user_table"class="table table-striped table-hover">
    ...
    <c:forEach items="${order}" var="u">
        <tr align="center" id="textx">
            ...
            <td>${u.name}</td>
            <td>${u.phone}</td>
            <td><a class="btn btn-warning" href="OrderServlet?option=findByIdOrder&id=${u.id}">修改</a>&nbsp;<a class="btn btn-danger" href="OrderServlet?option=deleteOrder&id=${u.id}">删除</a></td>
        </tr>
    </c:forEach>
</table>
